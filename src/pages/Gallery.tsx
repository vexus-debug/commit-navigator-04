...
const Gallery = () => {
  ...
  const paginated = filtered.slice((page - 1) * IMAGES_PER_PAGE, page * IMAGES_PER_PAGE);
      ...
      <section className="py-12 md:py-20 bg-background">
        <div className="container">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4 auto-rows-[240px] md:auto-rows-[280px]">
            {paginated.map((img, i) => (
              <Reveal key={img.src + active + page} delay={Math.min(i * 0.05, 0.3)} direction="scale" className={`${img.span || ""}`}>
                <button
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-10">
              <button
                ...
                disabled={page === 1}
                className="w-10 h-10 rounded-xl border border-border bg-background text-foreground flex items-center justify-center hover:bg-muted transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
          ...
          <p className="text-center text-xs text-muted-foreground mt-3">
            Showing {paginated.length} of {filtered.length} images{totalPages > 1 && ` — Page ${page} of ${totalPages}`}
          </p>
        </div>
      </section>
