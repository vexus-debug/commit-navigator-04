...
// Home: Mission Spotlight
export const HomeMissionSpotlightEditor = () => {
  const { content, updateContent } = useSiteContent();
  const save = useSave(["homeMissionSpotlight"]);
  const s = content.homeMissionSpotlight;
  return (
    <AdminSection title="Mission Spotlight" description="The image + quote section between 'Who We Are' and 'What We Do'.">
      <FormCard>
        <ImageInput label="Featured Image" value={s.image} onChange={(v) => updateContent("homeMissionSpotlight.image", v)} onRemove={() => updateContent("homeMissionSpotlight.image", "")} />
        <TextInput label="Image Caption" value={s.caption} onChange={(v) => updateContent("homeMissionSpotlight.caption", v)} />
        <TextArea label="Quote" value={s.quote} onChange={(v) => updateContent("homeMissionSpotlight.quote", v)} />
        <TextInput label="Quote Author" value={s.quoteAuthor} onChange={(v) => updateContent("homeMissionSpotlight.quoteAuthor", v)} />
      </FormCard>
      <SaveButton onClick={save} />
    </AdminSection>
  );
...
export const HomeSpotlightFeatureEditor = () => {
  const { content, updateContent } = useSiteContent();
  const save = useSave(["homeSpotlightFeature"]);
  const s = content.homeSpotlightFeature;
  return (
    <AdminSection title="Spotlight Feature" description="Full-width image section with overlay text (appears after the Journey section).">
      <FormCard>
        <ImageInput label="Background Image" value={s.image} onChange={(v) => updateContent("homeSpotlightFeature.image", v)} onRemove={() => updateContent("homeSpotlightFeature.image", "")} />
        <TextInput label="Tag" value={s.tag} onChange={(v) => updateContent("homeSpotlightFeature.tag", v)} />
        <TextInput label="Title" value={s.title} onChange={(v) => updateContent("homeSpotlightFeature.title", v)} />
        <TextArea label="Description" value={s.description} onChange={(v) => updateContent("homeSpotlightFeature.description", v)} />
        <div className="grid grid-cols-2 gap-3">
          <TextInput label="CTA Label" value={s.ctaLabel} onChange={(v) => updateContent("homeSpotlightFeature.ctaLabel", v)} />
          <TextInput label="CTA Link" value={s.ctaLink} onChange={(v) => updateContent("homeSpotlightFeature.ctaLink", v)} />
        </div>
      </FormCard>
      <SaveButton onClick={save} />
    </AdminSection>
